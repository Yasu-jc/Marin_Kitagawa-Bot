...
let handler = async (m, { conn }) => {
    ...
    let img;
    try {
      img = await conn.profilePictureUrl(who, 'image');
    } catch (e) {
      img = 'https://i.imgur.com/JqeuF6b.jpg'; // imagen por defecto si no tiene perfil
    }
    ...
    if (before !== user.level) {
        ...
        await conn.sendMessage(m.chat, { 
          ...
          caption: txt,
          buttons: [
            {buttonId: '.baltop', buttonText: {displayText: 'Ver Top Balance'}, type: 1}
          ],
          headerType: 4
        }, { quoted: m });
    } else {
